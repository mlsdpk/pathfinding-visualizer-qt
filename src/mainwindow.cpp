...
MainWindow::MainWindow(QWidget *parent)
    ...
    connect(ui->gridSlider, SIGNAL(valueChanged(int)), gridMap, SLOT(renderGrids(int)));
    ////////////////////////////////
...
// test slot (remove later)
void MainWindow::testDestroyed(QObject *obj) {
    std::cout << "Address of destroyed obj: " << obj << std::endl;
}

// this slot is called when the algorithm finshes and sends a signal
// of visited vertices in order
void MainWindow::receiveVisitedVerticesIdxOrder(const QList<int> *visitedVerticesIdxOrder) {
...
void MainWindow::receiveChildParentIdxs(const std::unordered_map<int, int> *childParentIdxs) {
    ...
    if (!animatePlanning_) {
        ...
        for (auto idx: visitedVerticesIdxOrder_) {
            if (idx != goalKeyIdx_)
                gridMap->getGridsList()->at(idx)->setBrush(QBrush(Qt::blue, Qt::SolidPattern));
        }
        // render path
        while (childParentIdxs_[currentPathKeyIdx_] != startKeyIdx_) {
            ...
            gridMap->getGridsList()->at(currentPathKeyIdx_)->setBrush(QBrush(Qt::gray, Qt::SolidPattern));
        }
    }
}
    ...
    if (visitedVerticesIdxOrder_.size() > 0) {
        if (animatePlanning_) {
            ...
            if (visualizationCurrentIdx_ <= visitedVerticesIdxOrder_.size()-1 ) {
                ...
                if (visitedVerticesIdxOrder_.at(visualizationCurrentIdx_) != goalKeyIdx_)
                    gridMap->getGridsList()->at(visitedVerticesIdxOrder_.at(visualizationCurrentIdx_))->setBrush(QBrush(Qt::blue, Qt::SolidPattern));
...
// all the necessary things effected by the algorithm changes should be added here
// including the signal and slots related to algorithms
void MainWindow::on_comboBox_currentTextChanged(const QString &arg1)
{
    ...
    connect(this, SIGNAL(sendGridMapInformation(const GridMap*)), planner_.get(), SLOT(plan(const GridMap*)), Qt::DirectConnection);
    connect(this, SIGNAL(sendResetStatus()), planner_.get(), SLOT(receiveResetStatus()), Qt::DirectConnection);

    // remove later
    connect(plannerThread_, SIGNAL(destroyed(QObject*)), this, SLOT(testDestroyed(QObject*)));

    // main function loop of the planner thread
    connect(plannerThread_, SIGNAL(started()), planner_.get(), SLOT(process()));

    // for quitting stuffs
    connect(this, SIGNAL(sendFinishStatus()), planner_.get(), SLOT(receiveFinishStatus()), Qt::DirectConnection);
    connect(planner_.get(), SIGNAL(finished()), plannerThread_, SLOT(quit()), Qt::DirectConnection);
    connect(plannerThread_, SIGNAL(finished()), plannerThread_, SLOT(deleteLater()));

    // for visualization stuffs
    connect(planner_.get(), SIGNAL(sendVisitedVerticesIdxOrder(const QList<int>*)),
            this, SLOT(receiveVisitedVerticesIdxOrder(const QList<int>*)), Qt::DirectConnection);
    connect(planner_.get(), SIGNAL(sendChildParentIdxs(const std::unordered_map<int, int>*)),
            this, SLOT(receiveChildParentIdxs(const std::unordered_map<int, int>*)), Qt::DirectConnection);
...
void MainWindow::on_resetButton_clicked()
{
    ...
    for (auto idx: visitedVerticesIdxOrder_) {
        if (idx != goalKeyIdx_)
            gridMap->getGridsList()->at(idx)->setBrush(QBrush(Qt::white, Qt::SolidPattern));
    }
