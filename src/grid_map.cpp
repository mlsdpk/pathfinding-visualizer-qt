...
void GridMap::setPlaceConfigMode(bool b) {
    placeConfigMode_ = b;
}

void GridMap::setEditingMapMode(bool b) {
    editMapMode_ = b;
}
...
const QList<Grid *> *GridMap::getGridsList() const
{
    return grids_;
}

Grid *GridMap::getGoalGrid() const {
    return goalGrid_;
}

Grid *GridMap::getStartGrid() const {
    return startGrid_;
}
...
void GridMap::renderGrids(int grid_size) {
    ...
    for (int col=0; col<no_of_row_grids_; col++) {
        for (int row=0; row<no_of_row_grids_; row++) {
            ...
            if (row > 0)
                grids_->at(node_idx)->setNeighbourIdx(col * no_of_row_grids_ + (row - 1));
            if (row < (no_of_row_grids_ - 1))
                grids_->at(node_idx)->setNeighbourIdx(col * no_of_row_grids_ + (row + 1));
            if (col > 0)
                grids_->at(node_idx)->setNeighbourIdx((col - 1) * no_of_row_grids_ + row);
    ...
    startGrid_->setStart();
    goalGrid_ = grids_->at(no_of_row_grids_*(no_of_row_grids_-1) + (no_of_row_grids_-1));
    goalGrid_->setGoal();
}

void GridMap::isGridClicked(Grid *grid) {
//    qDebug() << grid->pos().x() << " " << grid->pos().y();

//    // test function for find neighbours
//    for (const Grid* g: *(grid->getNeighbours())) {
//        qDebug() << g->pos().x() << g->pos().y();
//    }
...
//    qDebug() << "---";
    ...
    if (editMapMode_) {
        if (grid->isOccupied()) grid->setFree();
        else grid->setOccupied();
    }
